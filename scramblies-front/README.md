...
* [`src/scramblies_front/`](src/scramblies_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/scramblies_front/core.cljs): contains the SPA entry point, `init`
* [`test/scramblies_front/`](test/scramblies_front/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `scramblies-front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/scramblies_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns scramblies-front.example
  (:require [scramblies-front.config :as config])
